...
class Vis(QWidget):
    ...
    #---------------------------------------------------------------------------
    def ellipses(self, x, y, w,h, c='b', vmin=None, vmax=None, **kwargs):
...
# ec = EllipseCollection(ww, hh, aa, units='x', offsets=XY, offset_transform=ax.transData)
        ...
        zipped = np.broadcast(x, y, w,h)
        patches = [Ellipse((x_, y_), w_, h_)
                for x_, y_, w_, h_ in zipped]
        ...
        for child in cells_parent:
            ...
            for circle in child:  # two circles in each child: outer + nucleus
                ...
                if (np.fabs(yval) > too_large_val):
                    print("bogus xval=", xval)
                    break
#  <circle cx="560.565" cy="129.955" r="3.63387" stroke-width="0.5" stroke="cyan" fill="cyan"/>
# now:
#  <ellipse cx="160.821" cy="971.696" rx="50" ry="21.7945" stroke-width="0.5" stroke="black" fill="grey"/>
                # rval = float(circle.attrib['r'])
                rxval = float(circle.attrib['rx'])
                ryval = float(circle.attrib['ry'])
                # if (rgb[0] > rgb[1]):
...
#        print('fig.dpi=',fig.dpi) # = 72
        ...
        if (self.show_edge):
            try:
                ...
                self.circles(xvals,yvals, s=rxvals, color=rgbs, alpha=self.alpha, edgecolor='black', linewidth=0.5)
            ...
            # self.circles(xvals,yvals, s=rxvals, color=rgbs, alpha=self.alpha)
            self.ellipses(xvals,yvals, w=rxvals, h=ryvals, color=rgbs, alpha=self.alpha)
